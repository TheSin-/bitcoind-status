...
function getData($from_cache = false)
{
    ...
    if (!$data) {
        ...
        $return_data['display_load_chart'] = false;
        $return_data['display_memory_chart'] = false;
    ...
    if ($config['display_masternode_status'] === true) {
        ...
        if ($data['masternode_status_info']['status'] == "Masternode successfully started") {
            $vin = '';
            if (!empty($data['masternode_status_info']['vin']))
                $vin = preg_replace('/, /', '-', preg_replace('/., scriptSig=.*.$/', '', preg_replace('/^CTxIn.COutPoint./', '', $data['masternode_status_info']['vin'])));
            else if (!empty($data['masternode_status_info']['outpoint']))
                $vin = $data['masternode_status_info']['outpoint'];
    ...
    $data['display_load_chart'] = displayChart($config['display_load_chart'], $config['load_file'], $config['chart_min_data_points']);
    $data['display_memory_chart'] = displayChart($config['display_memory_chart'], $config['memory_file'], $config['chart_min_data_points']);
